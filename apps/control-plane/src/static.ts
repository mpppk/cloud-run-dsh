// Static delivery for the debug Web UI (issue #128) and the product UI
// (issue #138).
//
// Serves a fixed set of files from `apps/control-plane/public/` over an
...
// Dynamic pathnames such as `/app/<id>` are deliberately NOT served —
// the allowlist only matches exact paths (traversal stays impossible by
// construction: the file is chosen from a fixed map keyed by the exact
// pathname — the request string is never used to build a filesystem path).
//
...
const STATIC_ALLOWLIST: Readonly<Record<string, StaticEntry>> = {
  ...
  // Shared SSE parser (issue #138): served under both UI prefixes so each
  // page's relative `./sse.js` import resolves without a new build step.
  // Same file, two exact paths — still no catch-all, still no traversal.
  "/ui/sse.js": { file: "sse.js", contentType: "text/javascript; charset=utf-8" },
  // Product UI (issue #138): `/app` (home) and `/app?ws=<id>`
  // (conversation) share one document; the query string never reaches the
  // allowlist because matching is on `url.pathname` only.
