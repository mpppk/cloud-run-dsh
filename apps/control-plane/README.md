...
| `GCP_ACCESS_TOKEN` | no | metadata server | GCP OAuth2 token for the Instances API + GCS. On Cloud Run it is resolved from the metadata server automatically (the control-plane SA needs `roles/run.admin`); set explicitly only for local runs. Token handling is the shared `@cloud-run-dsh/gcp-token-provider` chain (metadata server → ADC → env, cached until 60s before expiry) used by both apps since #76 (origin: #27, PR #33). |
...
- Remote checkpoint trigger: the lifecycle checkpoint on `stop` prepares via
  the agent-host prepare-stop route (#72, #87), and manual checkpoints take a
  real host snapshot before writing the request marker (#75, #89).
