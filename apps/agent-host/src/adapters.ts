...
import type { GcsTokenProvider } from "@cloud-run-dsh/gcp-token-provider";
import type {
  SandboxCliResult,
  SandboxCliRunner,
...
// ---------------------------------------------------------------------------
// GCP access tokens — shared implementation.
//
// Issue #76: the metadata → ADC → env chain with caching/refresh moved to
// `@cloud-run-dsh/gcp-token-provider` so the control plane uses the same
// code (origin: issue #27, PR #33). This file keeps only the host's GCS
// client, which takes the provider as an injected seam.
// ---------------------------------------------------------------------------

// (Moved to @cloud-run-dsh/gcp-token-provider — see the section note above.)
...
export class BunSqlQueryExecutor implements SessionQueryExecutor {
   ...
   * the password never reaches the exception (issue #42). Construction goes
   * through the shared `createBunSqlClient()` so ambient `*_URL` variables
