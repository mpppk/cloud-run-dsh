...
import { restoreWorkspace, resolveWorkspaceEntryPath, parsePorcelainZ } from "./restore.js";
...
): FileSystem & { files: Map<string, Uint8Array>; unlinked: string[]; createdDirs: string[] } {
...
/**
 * Build `git status --porcelain -uall -z` style output.
 * Each record is XY + space + path + NUL, exactly as git emits it.
 */
function porcelainZ(entries: { xy?: string; path: string }[]): string {
  return entries.map((e) => `${e.xy ?? "??"} ${e.path}\0`).join("");
}

function makeRestoreGit(
  statusOutput: string,
  opts?: { applyExitCode?: number; statusArgs?: string[] },
  ...
  return {
    ...
    async run(args, _opts) {
      ...
      if (args[0] === "status") {
        if (opts?.statusArgs && args.slice(1).join(" ") !== opts.statusArgs.join(" ")) {
          return { stdout: "", stderr: "unexpected status args", exitCode: 2 };
        }
...
describe("restoreWorkspace", () => {
  test("clone -> checkout -> download -> git apply --binary -> extract untracked -> validate", async () => {
    ...
    const git = makeRestoreGit(porcelainZ([{ xy: " M", path: "modified.txt" }, { xy: "??", path: "new.txt" }]), {
      statusArgs: ["--porcelain", "-uall", "-z"],
    });
    const fs = makeFs();
  ...
  test("MAJOR-3: legit nested untracked entries still land inside the workspace", async () => {
    ...
    const git = makeRestoreGit(porcelainZ([{ xy: "??", path: "sub/dir/notes.txt" }]));
    const fs = makeFs();
  ...
  test("MINOR-9: extra untracked file in git status not present in manifest fails validation", async () => {
    ...
    const git = makeRestoreGit(porcelainZ([{ xy: "??", path: "known.txt" }, { xy: "??", path: "rogue.txt" }]));
  ...
  test("MAJOR-A: untracked file in a directory absent from the base commit gets mkdir -p before write", async () => {
    const storage = new InMemoryCheckpointStorage();
    storeRawBundle(storage, "chk-deep", {
      untrackedTar: createUntrackedTar([
        { path: "sub/deep.txt", content: new TextEncoder().encode("deep") },
        { path: "sub/deeper/deepest.txt", content: new TextEncoder().encode("deepest") },
      ]),
      untrackedFiles: ["sub/deep.txt", "sub/deeper/deepest.txt"],
    });
    const git = makeRestoreGit(porcelainZ([{ xy: "??", path: "sub/deep.txt" }, { xy: "??", path: "sub/deeper/deepest.txt" }]));
    const fs = makeFs(); // fresh clone: no sub/ exists
    await restoreWorkspace({
      workspaceDir: "/workspace",
      checkpointKey: "chk-deep",
      storage,
      git,
      fs,
      tmpDir: "/tmp-fake",
    });
    expect(fs.files.has("/workspace/sub/deep.txt")).toBe(true);
    expect(fs.files.has("/workspace/sub/deeper/deepest.txt")).toBe(true);
    // parents were created for every entry (mkdir -p semantics: both levels)
    expect(fs.createdDirs).toContain("/workspace/sub");
    expect(fs.createdDirs).toContain("/workspace/sub/deeper");
    // and every mkdir stayed inside the workspace
    for (const dir of fs.createdDirs) {
      expect(dir.startsWith("/workspace/")).toBe(true);
    }
  });

  test("MAJOR-A: traversal entry ../escape/x.txt cannot create a directory outside the workspace", async () => {
    const storage = new InMemoryCheckpointStorage();
    storeRawBundle(storage, "chk-evil-dir", {
      untrackedTar: createUntrackedTar([{ path: "../escape/x.txt", content: new TextEncoder().encode("evil") }]),
    });
    const git = makeRestoreGit("");
    const fs = makeFs();
    await expect(
      restoreWorkspace({ workspaceDir: "/workspace", checkpointKey: "chk-evil-dir", storage, git, fs, tmpDir: "/tmp-fake" }),
    ).rejects.toBeInstanceOf(RestoreValidationError);
    // no directory created anywhere, in or out of the workspace
    expect(fs.createdDirs.length).toBe(0);
    expect(fs.files.size).toBe(0);
  });

  test("MAJOR-B(a): untracked filename with spaces matches manifest (C-quoting avoided via -z)", async () => {
    const storage = new InMemoryCheckpointStorage();
    storeRawBundle(storage, "chk-space", {
      untrackedTar: createUntrackedTar([{ path: "my file.txt", content: new TextEncoder().encode("spacey") }]),
      untrackedFiles: ["my file.txt"],
    });
    // With -z, git emits the raw name (no C-quoting): `?? my file.txt\0`
    const git = makeRestoreGit(porcelainZ([{ xy: "??", path: "my file.txt" }]));
    const fs = makeFs();
    await restoreWorkspace({
      workspaceDir: "/workspace",
      checkpointKey: "chk-space",
      storage,
      git,
      fs,
      tmpDir: "/tmp-fake",
    });
    expect(fs.files.has("/workspace/my file.txt")).toBe(true);
  });

  test("MAJOR-B(b): file staged at checkpoint time (in patch, not in ls-files) is not an extra", async () => {
    const storage = new InMemoryCheckpointStorage();
    const patch = [
    ...
    storeRawBundle(storage, "chk-staged", {
      patchDiff: patch,
      untrackedTar: createUntrackedTar([]),
      untrackedFiles: [],
    });
    // After git apply, staged.txt shows up as untracked — but it came from the patch.
    const git = makeRestoreGit(porcelainZ([{ xy: "??", path: "staged.txt" }]));
    const fs = makeFs();
    await restoreWorkspace({
      workspaceDir: "/workspace",
      checkpointKey: "chk-staged",
      storage,
      git,
      fs,
      tmpDir: "/tmp-fake",
    });
    // no validation error: patch-introduced path is expected
  });

  test("MAJOR-B(c): untracked files under one directory are listed individually with -uall (no collapse)", async () => {
    const storage = new InMemoryCheckpointStorage();
    storeRawBundle(storage, "chk-dircollapse", {
      untrackedTar: createUntrackedTar([
        { path: "sub/a.txt", content: new TextEncoder().encode("a") },
        { path: "sub/b.txt", content: new TextEncoder().encode("b") },
      ]),
      untrackedFiles: ["sub/a.txt", "sub/b.txt"],
    });
    // -uall lists each file, not `?? sub/`
    const git = makeRestoreGit(porcelainZ([{ xy: "??", path: "sub/a.txt" }, { xy: "??", path: "sub/b.txt" }]));
    const fs = makeFs();
    await restoreWorkspace({
      workspaceDir: "/workspace",
      checkpointKey: "chk-dircollapse",
      storage,
      git,
      fs,
      tmpDir: "/tmp-fake",
    });
    expect(fs.files.has("/workspace/sub/a.txt")).toBe(true);
    expect(fs.files.has("/workspace/sub/b.txt")).toBe(true);
  });

  test("MAJOR-B: genuinely unexpected extra untracked file IS still rejected", async () => {
    const storage = new InMemoryCheckpointStorage();
    storeRawBundle(storage, "chk-real-extra", {
      untrackedTar: createUntrackedTar([{ path: "known.txt", content: new TextEncoder().encode("x") }]),
      untrackedFiles: ["known.txt"],
    });
    const git = makeRestoreGit(porcelainZ([{ xy: "??", path: "known.txt" }, { xy: "??", path: "totally-unexpected.txt" }]));
    const fs = makeFs();
    let err: unknown = null;
    try {
      await restoreWorkspace({
        workspaceDir: "/workspace",
        checkpointKey: "chk-real-extra",
        storage,
        git,
        fs,
        tmpDir: "/tmp-fake",
    ...
    expect(err).toBeInstanceOf(RestoreValidationError);
    const details = (err as RestoreValidationError).details as { unexpectedUntracked: string[] };
    expect(details.unexpectedUntracked).toEqual(["totally-unexpected.txt"]);
  });

  test("MINOR: unlink failure in finally does not mask the git apply error", async () => {
    const storage = new InMemoryCheckpointStorage();
    storeRawBundle(storage, "chk-unlink", { patchDiff: "broken patch", untrackedFiles: [] });
    const git = makeRestoreGit("", { applyExitCode: 1 });
    const fs = makeFs();
    fs.unlink = async () => {
  ...
  test("validation fails when clean expected but status dirty", async () => {
    ...
    const git: GitRunner = {
      async run(args) {
        if (args[0] === "status") return { stdout: porcelainZ([{ xy: " M", path: "dirty.txt" }]), stderr: "", exitCode: 0 };
        return { stdout: "", stderr: "", exitCode: 0 };
      },
    };
    await expect(
  ...
  test("missing untracked file after extraction fails validation", async () => {
    ...
    const git = makeRestoreGit(porcelainZ([{ xy: "??", path: "a.txt" }]));
