...
- **DB user after migrations (open issue [#73](https://github.com/mpppk/cloud-run-dsh/issues/73), measured 2026-09-05):** once the migration runner has applied `0001_init.sql`, its tables reference the `dsh_app` role, so deleting the `google_sql_user.app` user fails with `role "dsh_app" cannot be dropped because some objects depend on it` (400) and the destroy fails with it. If `terraform destroy` fails on the user, drop the objects owned by that role first via the proxy as in Step 4 (`psql "$DATABASE_URL" -c 'DROP OWNED BY dsh_app;'` — `DATABASE_URL` connects as `dsh_app`, and a role can always drop the objects it owns, so no superuser is needed) and re-run destroy. **Destructive, teardown-only: this permanently deletes every table `dsh_app` owns (all migrated data). Never run it against a database you intend to keep.** A failed destroy keeps billing — do not leave it.
