...
- **DB user after migrations (open issue [#73](https://github.com/mpppk/cloud-run-dsh/issues/73), measured 2026-09-05):** once the migration runner has applied `0001_init.sql`, its tables reference the `dsh_app` role, so deleting the `google_sql_user.app` user fails with `role "dsh_app" cannot be dropped because some objects depend on it` (400) and the destroy fails with it. If `terraform destroy` fails on the user, drop the migrated objects first via the proxy as in Step 4 (`psql "$DATABASE_URL" -c 'DROP SCHEMA public CASCADE; CREATE SCHEMA public;'` — `DATABASE_URL` connects as `dsh_app`, which owns the migrated tables, so no superuser is needed) and re-run destroy. A failed destroy keeps billing — do not leave it.
